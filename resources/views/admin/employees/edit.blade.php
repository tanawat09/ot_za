...
@section('header', 'แก้ไขข้อมูลพนักงาน (Edit Employee)')
    ...
    <div class="col-md-10 col-lg-8">
        <div class="card card-custom p-4">
            <h5 class="fw-bold font-heading mb-4 border-bottom pb-2">
                <i class="bi bi-pencil-square text-primary me-2"></i>แก้ไขพนักงาน: {{ $employee->full_name }}
            </h5>
            ...
            <form method="POST" action="{{ route('admin.employees.update', $employee) }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="supervisors[]" id="supervisors" class="form-select" multiple>
                            @foreach($supervisors as $spv)
                                <option value="{{ $spv->id }}" {{ in_array($spv->id, $selectedSupervisors) ? 'selected' : '' }}>
                                    {{ $spv->name }} ({{ $spv->email }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" id="status" class="form-select" required>
                            <option value="Active" {{ old('status', $employee->status) == 'Active' ? 'selected' : '' }}>Active (ปกติ)</option>
                ...
                <div class="d-flex justify-content-between mt-4 border-top pt-3">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="bi bi-save me-1"></i> อัปเดตข้อมูลพนักงาน</button>
                </div>
            </form>
        </div>
    </div>
